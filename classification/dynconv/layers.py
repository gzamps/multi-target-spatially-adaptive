...
## WEIGHTS ARE STORED LIKE: [ Cout, Cin, k1, k2]
...
def conv3x3_dw(conv_module, x, mask_dilate, mask, fast=False):
    w = conv_module.weight.data
    mask.flops_per_position += (w.shape[0]*w.shape[1]*w.shape[2]*w.shape[3]/conv_module.groups)
    conv_module.__mask__ = mask
    return conv_module(x)
...
def pwconv(conv_module, x, mask, fast=False):
    w = conv_module.weight.data
    mask.flops_per_position += w.shape[0]*w.shape[1]
    conv_module.__mask__ = mask
    return conv_module(x)

    # input.shape[1] * input.shape[2] * output.shape[3] * input.shape[3]  # Convnext linear piecewise, works
