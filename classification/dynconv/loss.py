...
class SparsityCriterionMulti(nn.Module):
    ''' 
    Defines the sparsity loss, consisting of two parts:
    - network loss: MSE between computational budget used for whole network and target 
    - block loss: sparsity (percentage of used FLOPS between 0 and 1) in a block must lie between upper and lower bound. 
    ...
    def __init__(self, sparsity_target, num_epochs):
        super(SparsityCriterionMulti, self).__init__()
        self.sparsity_target = sparsity_target
        self.num_epochs = num_epochs
    ...
    def forward(self, meta):
        ...
        # logger.add(str(meta['target_density'])+"/"+'flops', cost.item()/10e9)
        # logger.add(str(meta['target_density'])+"/"+'ttalflops', total/10e9)


        logger.add(str(meta['target_density'])+"/"+'flops_ratio', (cost/total).item())
        # logger.add(str(meta['target_density'])+"/"+'ttalflops', total/10e9)

        logger.add(str(meta['target_density'])+"/"+'upper_bound', upper_bound)
        logger.add(str(meta['target_density'])+"/"+'lower_bound', lower_bound)
        logger.add(str(meta['target_density'])+"/"+'cost_perc', perc.item())
        logger.add(str(meta['target_density'])+"/"+'loss_sp_block', loss_block.item())
        logger.add(str(meta['target_density'])+"/"+'loss_sp_network', loss_network.item())
        return loss_network + loss_block        